"""Multi-turn scheduling orchestration without external model requirements."""
...
class SchedulingAgent:
    def __init__(self, repository: EducationRepository, session_store: SessionStore):
        self.repository = repository
        self.session_store = session_store
        self.parser = SchedulingInputParser()
        self.service = SchedulingService(repository)

    def process(self, session_id: str, message: str) -> dict:
        message = (message or "").strip()
        cancellation = re.search(r"取消(?:课程)?\s*#?(\d+)", message)
        if cancellation:
            booking = self.repository.cancel_booking(int(cancellation.group(1)))
            if booking is None:
        ...
        if "确认" in message:
            return self._confirm_pending_booking(session_id, message)
        ...
        updates = {
            ...
            and (
                ...
                or "duration_minutes" in parsed.provided_fields
                or "duration_minutes" not in previous_state
            )
        }
        ...
        candidates = self.service.match_teachers(student_id=student.id, course_id=course.id, campus_id=campus.id, start_at=request.start_at, duration_minutes=request.duration_minutes, preferred_teacher_id=teacher.id if teacher else None)
        if not candidates:
        ...
        return {"category": "scheduling", "status": "matched", "answer": f"已查询教师档期，可选教师：{names}。请回复“确认 + 教师姓名”完成排课。", "candidates": [{"teacher_id": item.teacher.id, "teacher_name": item.teacher.name, "score": item.score, "reasons": list(item.reasons)} for item in candidates]}

    def _confirm_pending_booking(self, session_id: str, message: str) -> dict:
        ...
        for candidate_id in pending["candidate_teacher_ids"]:
            ...
            if teacher and teacher.name in message:
                teacher_id, teacher_name = teacher.id, teacher.name
                break
        if teacher_id is None and message in ("确认", "确认排课"):
            teacher_id = pending["candidate_teacher_ids"][0]
        ...
        try:
            booking = self.service.create_booking(
                ...
                campus_id=pending["campus_id"],
                start_at=datetime.fromisoformat(pending["start_at"]),
